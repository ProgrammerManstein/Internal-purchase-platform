...
export default {
  ...
  methods: {
    ...
    async getDetails(val) {
      let { data } = await this.$axios.post('/api/getproductdetails', {
        productID: val
      })
      // console.log(data)
      if (data.code == '200') {
    ...
    async addShoppingCart() {
      ...
      let { data } = await this.$axios.post('/api/addshoppingcart', {
    ...
    async addCollect() {
      ...
      let { data } = await this.$axios.post('/api/addcollect', {
