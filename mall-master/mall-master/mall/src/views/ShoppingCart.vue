  ...
  <div class="shoppingCart">
    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <!-- 购物车列表 -->
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="index">
          <div class="pro-check">
...
export default {
  ...
  methods: {
    ...
    // 修改商品数量的时候调用该函数
    async handleChange(currentValue, key, productID) {
      // 当修改数量时，默认勾选
      this.updateShoppingCart({ key: key, prop: 'check', val: true })
      // 向后端发起更新购物车的数据库信息请求
      let { data } = await this.$axios.post('/api/updateshoppingcartnumber', {
