<template>
  <div class="goods" id="goods" name="goods">
    ...
    <div class="main">
      <div class="list">
        <MyList :list="product" :classid="[0]" v-if="product.length>0"></MyList>
...
export default {
  data() {
    return {
      categoryList: [
        ...
        { category_id: 2, category_name: '日常用品' }
      ], //分类列表
      categoryID: [], // 分类id
      product: '', // 商品列表
      productList: '',
  ...
  methods: {
    ...
    async getData() {
      ...
      let { data } = await this.$axios.post('/api/getallproduct', {
    ...
    async getProductBySearch() {
      let { data } = await this.$axios.post('/api/getproductbysearch', {
        search: this.search
      })
      if (data.code == '200') {
        this.productList = data.data
